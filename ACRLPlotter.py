...
class ACRLPlotter:
    ...
    def __init__(self, numactions, numactors, numsigs, vsize):
        ...
        self.buff_gvfpredictions = numpy.zeros((self.vbufflen,1))
    ...
    def initPlot(self):
        ...
        self.li2gvfpredictions = self.ax2.plot(self.buff_gvfpredictions)
        ...
        for j in range(0, len(self.li2rewards)):
            ...
            plt.setp(self.li2gvfpredictions[j], linewidth = 2, color='red', drawstyle='steps-post',
                     alpha=1 - j / float(len(self.li2rewards) + 2), linestyle="--")
        ...
        self.t6 = figtext(0.13, 0.28, "MeanGVFPrediction100ts: " + str(0), fontsize=12, color="grey")
        plt.pause(0.05)
    ...
    def plotUpdate(self, plotData):
        ...
        self.gvfprediction = plotData['gvf_pred']
        ...
        gvf_predictions_t = numpy.array(self.gvfprediction)
        ...
        self.buff_gvfpredictions = numpy.vstack((self.buff_gvfpredictions, gvf_predictions_t.reshape(1, 1)))[1:, :]#GVF predictions
        ...
        min = numpy.zeros(3)
        max = numpy.zeros(3)
        min[0] =  self.buff_sigmas.min()
        min[1] =  self.buff_means.min()
        max[0] =  self.buff_sigmas.max()
        max[1] =  self.buff_means.max()


      #  self.ax3.set_ylim((min.min()-0.1, max.max()+0.1))
        self.ax3.set_ylim((min.min() - 0.1, max.max() + 0.1))
        # Plot REWARD
        ...
        for k in range(0, len(self.li2gvfpredictions)):
            self.li2gvfpredictions[k].set_ydata(self.buff_gvfpredictions[:, k])
            
        rewardmin = numpy.zeros(2)
        rewardmax = numpy.zeros(2)
        rewardmin[0] = self.buff_rewards.min()
        rewardmin[1] = self.buff_gvfpredictions.min()
        rewardmax[0] = self.buff_rewards.max()
        rewardmax[1] = self.buff_gvfpredictions.max()

        self.ax2.set_ylim((rewardmin.min() - 0.5, rewardmax.max() + 0.5))
