...
class ACRL:
    def __init__(self, continuous, gamma=1.0, lamb=0.4, alpha=0.1, size=10, numactions=2):
        self.gamma = 1.0
        self.alpha = 0.05#alpha/10#0.05#alpha#alpha/10
        self.beta = 0.4
        self.alpha_mu = 0.4
        self.alpha_sigma = 0.04
        self.alpha_r = 0.004
        self.lamb = 0.4
    ...
    def updateContinuous(self, currentState, nextState, Rtp1, At, gamma):
        ...
        self.R = Rtp1
        #Critic
        self.delta = self.R - self.Ravg + self.gamma * numpy.dot(self.nextState, self.w) - numpy.dot(self.currentState, self.w)
        self.Ravg = self.Ravg + self.delta * self.alpha_r
        self.e_w = self.lamb * self.e_w + self.currentState
        self.w = self.w + self.beta * self.delta * self.e_w
        #Actor
        ...
        self.e_sigma = self.lamb * self.e_sigma + ((At - self.mean)**2 - self.sigma**2) * self.currentState
        self.theta_sigma = self.theta_sigma + self.alpha_sigma * self.delta * self.e_sigma
        return self.delta


    def calcMuSigma(self,xt):
        self.mean = round(numpy.dot(self.theta, xt), 8)
        self.sigma = round(numpy.exp(numpy.dot(self.theta_sigma, xt)), 8)
    ...
    def getActionContinuous(self, xt):
        self.calcMuSigma(xt)
       # print 'mean is : ' + str(self.mean)
